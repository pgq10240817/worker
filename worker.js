...
function isEnabledFlag(value, defaultEnabled = false) {
...
const 是否开启值 = isEnabledFlag;

function normalizeConfigToggle(value, defaultEnabled = false) {
  return isEnabledFlag(value, defaultEnabled) ? 'yes' : 'no';
}
const 归一配置开关 = normalizeConfigToggle;

function getConfigBoolean(key, defaultEnabled = false, fallbackValue = undefined) {
  const defaultValue = fallbackValue !== undefined ? fallbackValue : (defaultEnabled ? 'yes' : 'no');
  return isEnabledFlag(获取配置值(key, defaultValue), defaultEnabled);
}
const 获取配置开关值 = getConfigBoolean;

function getConfigText(key, defaultValue = '', fallbackValue = undefined) {
  const value = 获取配置值(key, fallbackValue !== undefined ? fallbackValue : defaultValue);
  return value === undefined || value === null ? defaultValue : String(value);
}
const 获取配置文本值 = getConfigText;
...
function buildEffectiveConfig(config) {
  ...
  ['ev', 'et', 'ex', 'ech', 'ena', 'epd', 'epi', 'egi', 'ipv4', 'ipv6', 'ispMobile', 'ispUnicom', 'ispTelecom'].forEach(键 => {
    snapshot[键] = normalizeConfigToggle(snapshot[键], isEnabledFlag(配置默认值[键]));
  });
...
const 整理有效配置 = buildEffectiveConfig;

function readEnvConfigValue(env, ...nameCandidates) {
...
const 读取环境配置值 = readEnvConfigValue;
...
function getEnvConfigSnapshot(env = {}) {
  ...
  const snapshot = {};
  for (const [key, nameList] of Object.entries(mapping)) {
    const value = readEnvConfigValue(env, ...nameList);
    if (value !== undefined) snapshot[key] = value;
  }
  return snapshot;
}
const 获取环境配置快照 = getEnvConfigSnapshot;

function getEffectiveConfigSnapshot(env = {}) {
  return buildEffectiveConfig({
    ...getEnvConfigSnapshot(env),
    ...键值配置
  });
}
const 获取有效配置快照 = getEffectiveConfigSnapshot;
const 地区映射 = {
  'HK': ['🇭🇰 香港', 'HK', 'Hong Kong'],
...
function isValidUuidFormat(text) {
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  return uuidRegex.test(text);
}
const 是否有效格式 = isValidUuidFormat;
function isValidIpAddress(address) {
  const ipv4Regex = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
  if (ipv4Regex.test(address)) return true;
  const ipv6Regex = /^(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}$/;
  if (ipv6Regex.test(address)) return true;
  const compactIpv6Regex = /^::1$|^::$|^(?:[0-9a-fA-F]{1,4}:)*::(?:[0-9a-fA-F]{1,4}:)*[0-9a-fA-F]{1,4}$/;
  if (compactIpv6Regex.test(address)) return true;
  return false;
}
const 是否有效地址 = isValidIpAddress;
